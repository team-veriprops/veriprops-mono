...
- `backend-ci.yml` / `frontend-ci.yml` — PR quality gates (PRs to master/dev/staging), also reused by `deploy.yml` via `workflow_call` (so they have **no `push:` trigger** — adding one double-runs CI). Jobs are path-filtered via a `changes` job; branch protection must require only each workflow's always-reporting `summary` job.
- `e2e.yml` — full-stack drive-through (PRs to master, nightly, manual, and via `workflow_call` as the deploy gate). Mailpit runs as a named `docker run` container (not a GHA service) so the `messaging_retry` stage can stop/start it.
