...
- **Env & secrets.** Committed `.env.{env}` files (backend `dev_personal|test|dev|staging|prod`, frontend `.env|.env.test|.env.production` plus the deploy-time `.env.dev|.env.staging` that deploy.yml passes as `--build-env`/`--env` flags — Next.js can't auto-load custom names) are **config-only**. Secrets live in Doppler and are injected as process env vars, which pydantic-settings/Next.js give precedence over env files — locally via `doppler run -- …`, in deploys via the Doppler→Vercel sync. The credential key set is `Settings.SECRET_ENV_KEYS`; committed files keep those keys absent/empty/`CHANGE_ME`, enforced by `backend/test/unit/app/config/test_env_hygiene.py` (which also scans `frontend/.env*` and fails on provider-token-shaped values anywhere).
