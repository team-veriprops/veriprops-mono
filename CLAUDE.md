...
Product context lives in [MASTER-PRD.md](MASTER-PRD.md) — the single as-built consolidation. Read the relevant section before designing features that touch verification, agent onboarding, reports, or the WhatsApp channel (§26). Deferred work is one list, [§G Known Gaps & Roadmap](MASTER-PRD.md#g-known-gaps), mirrored in code as `TODO(gap):` markers — `grep -rn "TODO(gap)"` enumerates them all, and a new gap needs both halves. [PRD.md](PRD.md) is now only the orchestrator's per-cycle input slot.
...
- **WhatsApp channel (§26).** A second thin surface over the same backend, not a second backend. Inbound terminates at a signature-verified Meta webhook and joins the **existing** conversation pipeline, so it runs the same fraud scan and lands in the same admin console (Decision K) — never a parallel path. Money, documents, and reports stay on the website: chat hands off via **single-use RS256 links** (`/wa/{pay,upload,report}/[token]`, §26.5) that authorize one action on one case and are not sessions. Transport is chosen by `WHATSAPP_PROVIDER` (below). Business-initiated messages go out as **Meta-approved §26.7 templates**, whose definitions are code-owned and whose approval status is synced from Meta into an admin registry (D59) — Meta accepts free text only inside its 24-hour service window. An agent's reply typed in the console fans back out over the same transport: free text inside that window, and outside it the `window_reopen` template while the agent's own words stay queued until the customer's next message reopens the window (D72). **Milestone consent is enforced in the notification router, never at a send site** (§26.4.6/D65), and a milestone is addressed to the number the customer OTP-verified as theirs — never `users.phone`, which nobody proved control of over WhatsApp. A §26.4.5 **delegate** is the one grant beyond the account holder: one per case, status milestones only, revocable, and structurally incapable of reaching a report because the template it receives has no link parameter. **§26.10 analytics are counted from an append-only fact table** (`whatsapp_channel_events`, D80) written by a best-effort recorder at seven conversation seams — the channel's operational tables hold one mutated row per phone number and so cannot answer a *rate over a window*, which four of the seven metrics are. The read surface extends the existing `AnalyticsService` and `/admin/analytics` rather than adding a second metrics API (D86). Backend detail in [backend/CLAUDE.md](backend/CLAUDE.md); landings + widget in [frontend/CLAUDE.md](frontend/CLAUDE.md).
...
- **WHATSAPP_PROVIDER contract.** `WHATSAPP_PROVIDER=stub` records outbound messages in an in-process outbox and accepts injected inbound ones (`POST /dev/whatsapp/inbound`, `GET /dev/whatsapp/outbox`); `meta` is the live Cloud API. `ENVIRONMENT=test` requires `stub` — **CI and e2e never reach Meta** — and `ENVIRONMENT=prod` requires `meta`. Startup fails otherwise. Staging runs `meta` too, against Meta's developer test number (D88), which is why `WHATSAPP_PHONE_NUMBER_ID`/`WHATSAPP_BUSINESS_ACCOUNT_ID` carry **no defaults** and `meta` refuses to boot without them — a default could only be production's number. The two transports are exclusive with no fallback between them, so a live-send failure can never be silently absorbed by the stub. Never detect transport behaviour from `ENVIRONMENT` — read `WHATSAPP_PROVIDER`. The same switch selects the **template directory** (`providers/whatsapp/directory.py`), which reads §26.7 approval status back from Meta: the stub reports the declared set as approved so the channel stays demoable, and no automated run ever calls the Graph API.
