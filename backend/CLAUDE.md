...
**WhatsApp templates are plain text, not JSON.** `render_whatsapp_payload` renders the body directly rather than going through `render_model` (which JSON-parses, and is right only for the push payloads). Address a WhatsApp recipient with `MessageRequestRecipient.phone_digits` — Meta's `wa_id` form, digits with no `+` — never `phone.international_number`. `TODO(gap):` Meta-approved *template* sends (`template_name` + positional `template_variables`) are not wired: `WhatsappPayload.validate_content` rejects a template without its variables, and the per-template variable mapping belongs to the §7.7 registry (S8). Until then WhatsApp goes out as text, which the Cloud API accepts only inside the 24-hour service window.
