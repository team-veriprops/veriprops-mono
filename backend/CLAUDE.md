...
* **`0001_initial_schema` is frozen between squashes; new schema lands in additive migrations** (decision D49). `0001` creates a table only when it is absent, so a column added there would silently never appear on an already-migrated database. Additive revisions need no `table_exists` guard — alembic's version table already runs each one once, and the guard breaks offline (`--sql`) generation. `test_migration_schema_parity.py` reads builders from *every* migration in `versions/`.
* **Squashing the chain back into `0001` is a deliberate, occasional operation with one precondition: every live database must already be stamped at the head being folded in.** `0001`'s `revision` id is the *last folded revision*, never the filename (`0011_whatsapp_legal_copy` today; `0003_user_verify_started` before that) — that is what makes an already-migrated database match the squashed file and do nothing, while a fresh one builds the same schema in one step. So the order is: migrate **every** environment (dev, staging, prod, test) to head, *then* squash. Squashing first would strand any database still on an older revision, because the intermediate revisions it needs no longer exist. Fold `add_column` steps into the relevant `CREATE TABLE` **after** the `*AlembicUtils.base_audit_columns()` splat, mirroring where `ADD COLUMN` actually puts them, so a freshly built database matches a migrated one column-for-column. Prove it before deleting the old files: build one database through the chain and one from the squash, then `diff` their `pg_dump --schema-only` output and their seed rows.
...
**The §26.7 Meta template contract (D59).** Meta will not deliver a business-initiated WhatsApp message as free text, so a declared template must send *as a template*:
...
- **Template-by-registration.** A declared template always sends as a template (all seven §26.7 entries are business-initiated, hence outside the window); free text is only for in-conversation bot replies, which are inside the window by construction. The rendered body still travels in `text` so the console, the stub outbox and the drive-through can read it. `WhatsAppWindowService` (`channel/whatsapp/window.py`) answers the one genuinely open case — an agent replying hours later — and reads *closed* when in doubt.
- **Approval status is Meta's, and advisory.** `whatsapp_templates` is synced from `GET /{waba_id}/message_templates` via the `WHATSAPP_PROVIDER`-selected directory facade (`providers/whatsapp/directory.py`; the stub reports the declared set approved, so CI/e2e never reach Meta). An unreadable or absent status becomes `NOT_FOUND`, never `APPROVED`. It surfaces in `/admin/config/whatsapp-templates` and on the §26.11 gate — it never blocks a send, because a stale sync must not take the channel down.
...
### Conversational channels (`app/domain/channel/`, PRD §26)

A channel is a thin surface over the same services the website uses — no case state lives there.

- **Transport** is in `appodus_utils/integrations/messaging/providers/whatsapp/`: the live Cloud API provider, the deterministic `stub` (records to an in-process outbox), the `webhook` receiver, `inbound.py` (Meta envelope → `InboundWhatsAppMessage`), and `outbound.py` (rules both transports enforce, e.g. the no-outbound-voice ban, §26.1.5). Selected by `WHATSAPP_PROVIDER`; the two routing rules are exclusive, never a fallback chain. **Per environment (D88):** test and dev are `stub` (test by boot-enforced contract), **staging is `meta` against Meta's developer test number**, prod is `meta` on the official number. That is why `WHATSAPP_PHONE_NUMBER_ID`/`WHATSAPP_BUSINESS_ACCOUNT_ID` have **no defaults** and `_enforce_whatsapp_identity_policy` refuses to boot on `meta` without them — they once defaulted to production's, which a stubbed staging hid and a live staging would have turned into QA traffic sent from the official number. Never reintroduce a default for either.
...
- **The bot's dispatch order is its safety model** (`channel/whatsapp/bot/engine.py`, §26.6). Every turn runs the same gauntlet and the order is the guarantee: human-on-thread (silent, D57) → welcome (§26.6.1) → **guardrails on the customer's own words** → classifier → guardrails on the classified intent → §26.3.4 capability → unmatched counter. `guardrails.py` is deterministic and sits **outside** the classifier (D44) — that ordering is what makes "a model cannot talk the bot into rendering a verification judgment" true rather than hoped for, so never move a guardrail behind the classify call. Anything that raises becomes a warm handover plus a `BOT_PIPELINE_FAILED` admin notification (§26.6.5): a silent bot is indistinguishable from a scam that stopped replying.
- **The engine is a dispatcher; the rules live in their own modules.** `guardrails.py` (never-answerable), `capabilities.py` (§26.3.4 as data — pay/upload/report are `HANDOFF`, account management is `NOT_OFFERED`), `content.py` (every word, code-owned per D54, with **pricing rendered from `PricingConfigService`** — never a literal), `projection.py` (the single owner of §26.3.2's ten stage names, *projected* from `VerificationStatus`, never stored), `support_hours.py` (Decision G coverage from `system_config`, in WAT), and `flows/` (pure functions over data the engine already fetched, so the §26.6.4 adversarial suite needs no stack). Customer-facing status wording comes from `verification/tracking/labels.py` — the projection's names are the channel's internal vocabulary, not a second thing to show a customer.
...
- **The intent facade is provider-swappable and fails to `UNKNOWN`** (`appodus_utils/integrations/intent/`, D53/D87). `INTENT_PROVIDER` selects stub (deterministic keyword table) / anthropic (SDK, forced tool-use) / openai_compatible (raw httpx, base-URL + model + key); `ENVIRONMENT=test` **requires** the stub, so no automated run ever calls a model. **The live default is `openai_compatible` pointed at DeepSeek** (`deepseek-chat`, `https://api.deepseek.com` — the adapter appends `/chat/completions`, so no `/v1`), in prod and staging alike. Never `deepseek-reasoner`: one closed-set label under `INTENT_TIMEOUT_SECONDS` gains nothing from thinking tokens and loses the turn to the timeout. The anthropic adapter is kept rather than deleted — a facade with one live implementation is not a facade, and reverting the default is then one env var. Every adapter resolves its own failures — timeout, bad key, malformed or off-vocabulary output — to `BotIntent.UNKNOWN` rather than raising, and `IntentService` applies the one confidence gate (`INTENT_MIN_CONFIDENCE`). `coerce_intent` closes the vocabulary: a model may not return `STOP_MESSAGES` or `CONTINUE_CASE`, which are keyword-only because they revoke consent or claim a case.
...
- **Messaging consent is enforced in the router, never at a send site** (§26.4.6, D63/D65). `channel/whatsapp/consent/` holds one row per account carrying a **grant and a revoke timestamp** for each of the two §26.4.6 opt-ins, plus the capture point (`WhatsAppConsentSource`) that set them. Granted-ness is **derived** from the pair, never stored: §26.8 wants the record timestamped and exportable, so the timestamps *are* the record and a boolean beside them could only drift from the evidence justifying it (the D73 argument in a second place). Absence of a row means both off — §26.4.6 requires unticked defaults, and silence is never consent.
- **A milestone passes three gates, all outside the send site** (`channel/whatsapp/milestones.py`, D65): `ENABLE_OUT_MESSAGING`, the D63 utility consent, and an **ACTIVE linked number** resolved through `WhatsAppLinkService.resolve_phone_for_user` — the outbound inverse of `resolve_user_for_phone`, and the only address a business-initiated message may use. Never `users.phone`: that is a profile field nobody proved control of over WhatsApp, so addressing case details to it is §26.4.3's leak running outwards. A closed gate returns rather than raising, and the whole send is best-effort: a milestone is a courtesy on top of a state change that already happened, and must never roll back the transaction that made it.
- **`NotificationRule` carries WhatsApp as its own channel with its own template** (D65). `whatsapp: bool` + `whatsapp_template` — the §26.7 Meta template is not the email template and one field cannot be both. The row's recipient is resolved elsewhere (the link, not the profile), which is an asymmetry D65 names rather than hides. `whatsapp=True` means two things at once: the customer gets that template, **and** the case's authorized delegate gets `delegate_status` (§26.4.5). Exactly four rows carry it, and `test_notification_service.py` fails if a fifth appears — a stray flag would send a business-initiated template Meta never approved for that trigger.
...
- **`report_ready` carries the portal deep link, not a handoff token** (D75). A §26.5 token lives fifteen minutes; a milestone is read whenever the customer next opens WhatsApp, so a token would send most readers to the expiry page from a message they never clicked. Decision B already puts the report behind a login. The bot still mints a fresh `report` token **on request**, which is §26.4.2's recovery path used where it belongs.
- **§26.3.4's three `HANDOFF` actions now all have producers** (S10.0, WA-17). `upload` always did — a document arriving is something that happens *to* the bot (§26.6.3) — but `pay` and `view report` are things a customer **asks** for, and there was no `BotIntent` for either: both fell through to the unmatched counter, `capabilities.handoff_intent_for` had zero callers, and the `/wa/pay/<token>` and `/wa/report/<token>` landings were built, tested and unreachable from a real conversation. `bot/flows/handoff.py` is §26.6.3's shape for the other two, reusing `status.choose_prompt`/`status.resolve_choice` so all three "which case?" questions accept the same answers. **Eligibility reads off the §26.3.2 projection, and the report rule is narrower than it looks:** `ChannelState.REPORT_READY` projects from `UNDER_REVIEW`, where the report exists but has not passed the §8 release gate, so `VIEW_REPORT` offers `DELIVERED` only — a link at `REPORT_READY` would hand the customer the very draft the gate holds back. `PAY` covers `INTAKE_COMPLETE` **and** `PAYMENT_PENDING`: the first projects from `SUBMITTED`, which is where a customer asking "how do I pay?" almost always is, and naming only the second reads right while offering nothing to nearly everyone who asks (the live drive-through caught exactly that).
- **STOP is answered by the bot, in one turn, always** (D64). The keyword sets are matched on the customer's literal words *before* the classifier, and `STOP_MESSAGES`/`START_MESSAGES` are excluded from `CLASSIFIABLE_INTENTS` so a model can never revoke someone's consent by inference. STOP revokes **both** consents; START restores **utility only**, and the reply says so — marketing re-consent is a deliberate evidenced act on the web, because it is the consent §26.10 counts as a growth asset. An opt-out that waits for a person is not an opt-out: routing STOP to a human means the next milestone still sends outside G1 hours.
- **A delegate is a second identity lookup, beside the first — never inside it** (§26.4.5, `verification/delegate/`, D67). The bot resolves an inbound number as: `resolve_user_for_phone` (the channel's single **account** lookup, and therefore its single audit surface), then `CaseDelegateService.resolve_delegate_for_phone`, then stranger. The order is the access control: a number that is both resolves as the **customer**, because the account grant is strictly wider and reading it as a delegate would lose that person their own data. `case_delegates` carries its own `phone_e164` and never writes `whatsapp_links` — reusing the link table would break its 1:1 constraints or hand a delegate an account-shaped identity.
...
- **The channel is inside the §4.11 erasure and the §19.3 audit pack** (S11, WA-42). `PiiPseudonymiser` covered eight surfaces and none of them was a channel table, so an approved erasure left the subject's number in `whatsapp_links`, `whatsapp_bot_sessions`, `whatsapp_inbound_messages`, `case_delegates` and `handoff_token_redemptions` — and their half-finished chat intake, addresses and all, in the session `context`. It now scrubs by **phone number**, which means reading the subject's links *before* clearing them. The line it draws is §26.8's own: **identity is severed, content is retained** — chat logs are retained business records, so what a customer said stays while the number, the Meta profile name and the raw envelope carrying both do not. `whatsapp_links.phone_e164` goes to NULL rather than a token (a retained value would hold the unique constraint and lock that number out of every future account, the same reason `release_number` exists), and `whatsapp_inbound_messages.from_phone` takes the flat redaction because the column is 20 characters and the subject token is 23. The §26.4.6 consent ledger exports in the audit pack as one row per opt-in, emitted from the **timestamp pair** rather than the derived boolean — the pair is what §26.8 means by "timestamped and exportable", and a customer who was never asked produces no row rather than one claiming they declined.
- **§26.10 is counted from an append-only fact table, not from the channel's own state** (`channel/whatsapp/analytics/`, D80). Four of the seven metrics are *rates over a window*, and `whatsapp_bot_sessions` cannot answer one: it holds one **mutated** row per number, so `last_escalation_reason` is overwritten on every escalation and `current_flow` is cleared when a flow ends — a finished intake reads identically to one that never started. `audit_logs` was the other candidate and is the wrong home (it is the legal ledger the §19.3 pack exports). `ChannelEventRecorder.record` is **best-effort and never raises**, for the same reason `bot/sender.py` is — which is also why a broken call site shows up as a dashboard of zeros and no error, and why `_run_channel_analytics_checks` reads the numbers back after driving real traffic. Each of the seven facts is written at the **single** place it can occur: `ESCALATED` lives in `note_escalation`, which all nine `EscalationReason`s already funnel through, so a tenth reason is counted the day it is added. `PAYMENT_COMPLETED` comes from an `EventType.PAYMENT_CONFIRMED` subscriber that records only for cases the channel already wrote a fact about — every payment on the platform passes through it, and counting them all would report a seam conversion well over 100%.
- **The §26.4.1 page code is lifted out at `ingest`, never in the Meta normalizer** (D85). `[ref: web-home]` is metadata the customer's phone typed for them; left in the text it reaches the intent classifier, the guardrails, and an agent reading a message their customer did not write. It is extracted in `WhatsAppInboundService.ingest` because that is the single funnel **both** doors pass through — Meta's webhook and `POST /dev/whatsapp/inbound`, which builds its message directly and skips the normalizer. Extracting upstream gave the two doors different behaviour, so attribution worked in production and never in an automated run. `payload` keeps Meta's original untouched.
- **Meta's number quality rating is cached, and nothing in the send path reads it** (`analytics/health_service.py`, D81). The §26.7 template registry's posture applied to the other thing Meta knows about us: their verdict, our `last_synced_at`, and a `sync_error` kept rather than raised. Gating sends on a RED rating would take the channel down on a number we cannot verify; gating on a *stale* one would take it down because a Graph call timed out. A failed sync **keeps the previous rating** rather than downgrading it to `UNKNOWN` — a network blip is not Meta withdrawing its verdict.

## Dev/QA endpoints (non-production only)

`POST /dev/reset` + `POST /dev/seed` (`app/domain/dev/`) are the automation-determinism contract: reset clears domain data (keeping the super-admin + reference seeds), seed builds a deterministic scenario (customer + approved agents + an `UNDER_REVIEW`, SLA-overdue verification). `GET /dev/messages/latest?recipient=` (bookkeeping snapshot of the newest outbound message) and `POST /dev/messages/rewind?recipient=&rewind_expiry=` (pulls `next_retry_at`/`expires_at` into the past so the retry sweep fires immediately against the default backoff ladder; touches only those timestamps) extend the same contract for the messaging-retry pipeline. `POST /dev/whatsapp/inbound` (deliver an inbound message as if Meta had posted it — it enters the real ingestion path, not a shortcut), `GET|DELETE /dev/whatsapp/outbox` (read/clear what the stub transport recorded), `POST /dev/whatsapp/handoff-token` (mint a §26.5 link before the bot flows that issue them exist), `POST /dev/whatsapp/rewind-window?phone=&hours=` (age a number's inbound journal so Meta's 24-hour window reads as closed — the window is derived from when the customer last wrote, and a test does not get to wait a day; touches only `received_at`, the same trick and justification as `/dev/messages/rewind`), and `POST /dev/whatsapp/fail-next-turn` (arms **one** bot turn to raise, via `app/core/fault_injection.py`, so §26.11's "kill the bot, observe the auto-reply + alert" gate is drilled on a live stack rather than asserted against a mock — one-shot, so a forgotten arm cannot silence an environment, cleared by `/dev/reset`, and `arm()` raises in production on top of the router gate) do the same for the WhatsApp channel. **Production-gated twice** — the router only mounts when `settings.ENVIRONMENT != PRODUCTION`, and `_require_non_prod()` 404s in prod. Never remove either guard. The committed live drive-through `backend/scripts/e2e_drive_through.py` runs one cradle-to-grave scenario over HTTP against a running server — session-refresh contract (silent re-auth: happy refresh-and-retry + CSRF/missing-cookie/revoked-session failures) → fresh signup (referral code) → agent onboarding/KYC-stub + admin-invitation RBAC → draft/quote/submit → stub payment → assignment/execution (evidence) → chat fraud-hold approve+reject/SLA → review (reject/rework, release gate) → release → report PDF → tracking/SSE → sharing → dispute/re-check/upgrade → payouts → PREMIUM finish (LAWYER task, v2/v3 reports, declined re-check, upheld dispute) → referral earn+spend → pricing/analytics/broadcast hardening → pool/no-show/starvation + pause/delay/cancel/fail + chargeback (on the seeded "ops" verification) → audit pack + erasure reject/execute → the WhatsApp channel (`whatsapp`: signature-verified Meta webhook with tampered/unsigned/wrong-secret rejections, redelivery acknowledged-and-deduped, console inbound under the same fraud scan, a handoff link carried through to a PAID case, an agent's console reply arriving on the customer's phone, the 24-hour window closed by `rewind-window` so the `window_reopen` nudge and the reply queue are both exercised, §26.6.3's non-text answers including the `upload` link a document earns, the §26.3.4 pay and report handoffs a customer asks for in words, §26.10's seven metrics read back after the traffic above has moved them, and §26.6.5's failure drill) → Mailpit email delivery + password reset → outbound-message failure→retry→threshold→expiry (`messaging_retry`, last: stops/starts the Mailpit container via the docker CLI to induce a real SMTP failure, drives `POST /messages/sweeps/retries` with `/dev/messages/rewind` for interval-independence). Stages live in `backend/scripts/e2e/` (shared harness in `e2e/harness.py`, which counts checks so the runner prints a total rather than leaving it to be grepped out of the `[PASS]` lines); `--stages` runs a contiguous prefix of the stage order (stages have linear data dependencies). Best coverage: `docker compose up -d mailpit` and run the backend with `ENABLE_OUT_MESSAGING=True` (email → Mailpit, SMS → mock provider); without Mailpit (or docker CLI) the email + messaging_retry stages warn-skip and the rest still passes. The `whatsapp` stage's **signature** checks need `WHATSAPP_APP_SECRET_KEY` + `WHATSAPP_BUSINESS_WEBHOOK_VERIFY_TOKEN` exported to **both** the backend and the script (they are Doppler-managed and absent from committed env files); without them it warn-skips the webhook and injects through the dev endpoint instead, so everything downstream is still covered.
...
**Three layers, and they answer different questions.** Reach for the cheapest that can actually fail on the change you made — but note which questions the unit layer structurally *cannot* answer, because that is where this codebase's defects have historically come from.

- **`test/unit/`** — fast, in-memory, no server. Two top-level mirrors, matching the two-layer package: `test/unit/app/` mirrors `main/app/` (one directory per domain) and `test/unit/appodus_utils/` mirrors `main/appodus_utils/` (common, config, db, middleware, integrations). A new domain's tests go in the mirror position of the module they cover; a new directory needs an `__init__.py`. `test/utils/` holds `mock_circuit_breaker.py` and shared fixtures, `test/fixtures/` holds sample payloads (scraper HTML).
- **`backend/scripts/e2e_drive_through.py`** — the live API drive-through described under "Dev/QA endpoints", over HTTP against a running server. This is the layer that catches **seam** defects: an unimported webhook handler, a per-channel send failure swallowed two frames up, a re-fetch inside an uncommitted transaction, a bot capability with no intent reaching it. Every one of those passed its unit tests. If a change crosses a service boundary, a transport, or a transaction, it needs a check here.
- **`frontend/e2e/`** — the Playwright browser UAT suite ([docs/uat-strategy.md](../docs/uat-strategy.md)). Not wired into CI; run against a live local stack over **HTTPS**.

Conventions:

- `pytest.ini` sets `asyncio_mode = auto`, so async tests need no decorator; `conftest.py` defaults `APPODUS_ACTIVE_ENV=test` before any `main.*` import.
- A `@transactional` service method needs a session in context — set `db_session_ctx` from an autouse fixture, and build the service with `object.__new__(Service)` plus mocked collaborators rather than going through DI (`test/unit/app/domain/channel/whatsapp/test_window_service.py` is the shape).
- Mailpit captures email in dev/local/test; the drive-through's email + `messaging_retry` stages warn-skip without it.
- Guard tests exist to make a convention enforceable rather than remembered — schema/builder parity, wire casing, env hygiene, webhook-handler registration, the `NotificationRule` WhatsApp-flag count. Extend the guard's list when you add the thing it guards; do not weaken it to make a change pass.