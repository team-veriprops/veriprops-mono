...
- `TODO(gap):` the authentication-template **button** component (copy-code / one-tap) is not specified in Meta's public send-side docs, so it is declared per template and `otp_auth` ships body-only — confirm at the S11 live smoke.
