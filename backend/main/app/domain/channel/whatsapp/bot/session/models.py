"""Per-conversation bot state (PRD §26.3.3, §26.6, WA-11).

One row per WhatsApp number — the "state-per-conversation" §26.3.3 asks for. It holds only
...
class BotSessionDto(Object):
    ...
    ``window_open`` is the second thing an agent needs *before* typing (§26.7): outside
