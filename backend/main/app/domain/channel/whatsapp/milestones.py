"""§7.6.2 milestone delivery over WhatsApp (D65; WA-16/WA-34/WA-35).
...
2. **The customer granted the §7.4.6 utility consent.** Read from the D63 ledger.
3. **The account has an ACTIVE linked number.** Resolved through
   ...
   nobody proved control of over WhatsApp would be the §7.4.3 leak running outwards.
...
def status_label_for(verification: Verification) -> str:
    """The one customer-facing status vocabulary (§7.4.5 delegate copy).

    `verification/tracking/labels.py` is what the dashboard renders, so a delegate and the
    buyer read the same words — the §7.3.2 projection's stage names are the channel's
...
class WhatsAppMilestoneSender:
    ...
    async def send_customer_milestone(
        ...
        """Deliver one §7.7 milestone to the customer, if all three gates pass."""
        ...
        if not await self._whatsapp_consent_service.utility_granted(user_id):
            # Not a failure — this is the §7.4.6 opt-in doing its job.
            return
    ...
    async def send_delegate_milestone(
        ...
        """Deliver the §7.4.5 delegate's status-only milestone.
    ...
    def _context_for(
        ...
        `report_ready` carries the **portal deep link** rather than a §7.5 handoff token
        ...
        `report` link on request — which is §7.4.2's recovery path, used when it is needed
