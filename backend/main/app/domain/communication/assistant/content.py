"""Everything the assistant says (PRD §26.6, §16.7, D54).
...
from main.app.domain.communication.assistant.support_hours import Coverage
...
_ESCALATION_OPENINGS: dict[EscalationReason, str] = {
    ...
    EscalationReason.CAPABILITY_NOT_OFFERED: (
        "I can't do that from this chat, but a team member can help you with it."
    ),
...
def fresh_intake_link(link: str) -> str:
    """WhatsApp — a re-issued single-use link for answers the assistant already holds."""
    return (
...
# ─── In the portal (§16.7, D93) ─────────────────────────────────────
#
# The reader is signed in, so there is no number to link and no link that expires: every
# handoff is a plain link to the page behind their own login. The pledge still rides on
# anything that leads to a payment — an impersonator can copy a portal chat as easily as a
# WhatsApp one.

PORTAL_STATUS_FOOTER = "Open the verification from your dashboard for the full details and your report."


def portal_identity_required() -> str:
    """"Link my account" asked in the portal, where the account is already the one signed in."""
    return (
        "You're signed in, so there's nothing to link — I can already see your "
        "verifications.\n\n"
        "Ask me for your status, pricing, or to start a verification. Or reply 4 to talk to "
        "a person."
    )


def pay_with_portal_link(link: str) -> str:
    return (
        "You can pay for this verification here:\n"
        f"{link}\n\n"
        f"⚠️ {PAYMENT_PLEDGE}"
    )


def report_with_portal_link(link: str) -> str:
    return f"Your report is ready here:\n{link}"


def intake_portal_closing(link: str) -> str:
    """The web intake's closing: the answers are already a draft, waiting to be confirmed."""
    return (
        "Got everything I need. 👍\n\n"
        "I've started a verification with those details. Check them and pay here:\n"
        f"{link}\n\n"
        f"⚠️ {PAYMENT_PLEDGE}"
    )


def intake_portal_link_again(link: str) -> str:
    return f"Your details are saved in a draft verification — pick it up here:\n{link}"
