...
# The last line of a one-case answer where the reader is not signed in (WhatsApp). A surface
# whose reader is already behind their login says something else.
SIGN_IN_FOOTER = "Sign in at veriprops.ng for the full details and your report."
...
def render(cases: Sequence[CaseSummary], footer: str = SIGN_IN_FOOTER) -> StatusOutcome:
    """The reply for a "check my status" turn. *footer* is the surface's closing line."""
    if not cases:
        return StatusOutcome(_no_cases())
    if len(cases) == 1:
        return StatusOutcome(_one_case(cases[0], footer))
...
def render_choice(
    cases: Sequence[CaseSummary], choice: str, footer: str = SIGN_IN_FOOTER
) -> Optional[StatusOutcome]:
    ...
    return StatusOutcome(_one_case(selected, footer)) if selected else None
...
def _one_case(case: CaseSummary, footer: str) -> str:
    lines = [
