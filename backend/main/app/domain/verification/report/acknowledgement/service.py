...
class ReportAcknowledgementService:
    def __init__(self, repo: ReportAcknowledgementRepo):
        self._repo = repo

    async def is_acknowledged(
        self, customer_id: str, verification_id: str, report_version: int
    ) -> bool:
        return await self._repo.get_for(customer_id, verification_id, report_version) is not None
    ...
    async def acknowledge(
        ...
        existing = await self._repo.get_for(customer_id, verification_id, report_version)
        if existing is not None:
            return existing
        return await self._repo.create_return_model(CreateReportAcknowledgementDto(
