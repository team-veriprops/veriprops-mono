...
def _session(**overrides) -> AssistantSession:
    ...
    session.conversation_id = "c0ffee00000000000000000000000001"
...
# Where each collaborator now lives. The engine is surface-neutral (D93): identity, links,
# delivery, consent and §26.10 facts moved to the WhatsApp surface. These tests are about
# the words a WhatsApp customer reads, so they drive the pair through the surface's own
# `handle` and reach collaborators by their historical names.
_SURFACE_COLLABORATORS = {
    "_whatsapp_link_service", "_whatsapp_consent_service", "_case_delegate_service",
    "_handoff_token_service", "_channel_events", "_sender",
}


class _Turn:
    """The engine and its WhatsApp surface, addressed as one object."""

    def __init__(self, engine: AssistantEngine, surface: WhatsAppAssistantSurface):
        object.__setattr__(self, "engine", engine)
        object.__setattr__(self, "surface", surface)

    async def handle(self, message, conversation):
        return await self.surface.handle(message, conversation)

    def __getattr__(self, name):
        owner = self.surface if name in _SURFACE_COLLABORATORS else self.engine
        return getattr(owner, name)

    def __setattr__(self, name, value):
        if name == "_verification_repo":
            # Both read cases: the engine lists them, the surface mints links for one.
            setattr(self.engine, name, value)
            setattr(self.surface, name, value)
            return
        setattr(self.surface if name in _SURFACE_COLLABORATORS else self.engine, name, value)
...
def _engine(
    ...
    *,
    ...
    """An engine and WhatsApp surface wired to fakes, plus the list of replies sent."""
    sent: list[str] = []

    engine = object.__new__(AssistantEngine)
    surface = object.__new__(WhatsAppAssistantSurface)
    surface._engine = engine

    session_service = object.__new__(AssistantSessionService)
    session_service._assistant_session_repo = _FakeSessionRepo(session)
    engine._sessions = session_service
    surface._sessions = session_service
    ...
    surface._channel_events = _FakeChannelEventRecorder()
    ...
    sender.reply = _reply
    surface._sender = sender

    surface._whatsapp_link_service = MagicMock()
    surface._whatsapp_link_service.resolve_user_for_phone = AsyncMock(return_value=user_id)
    ...
    verification_repo.page_for_customer = AsyncMock(return_value=(list(cases), len(cases)))
    engine._verification_repo = verification_repo
    surface._verification_repo = verification_repo
    ...
    surface._whatsapp_consent_service = MagicMock()
    surface._whatsapp_consent_service.revoke_all = AsyncMock()
    surface._whatsapp_consent_service.grant_utility = AsyncMock()
    ...
    surface._case_delegate_service.resolve_delegate_for_phone = AsyncMock(return_value=None)
    surface._case_delegate_service.revoke_by_phone = AsyncMock(return_value=None)

    return _Turn(engine, surface), sent
