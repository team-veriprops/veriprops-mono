...
Small, but it earned its own file. `AssistantSessionService` is wrapped by
...
from main.app.domain.communication.assistant.session.service import to_assistant_session_dto

NOW = datetime(2026, 9, 1, 12, 0, tzinfo=timezone.utc)


def _row(**overrides) -> AssistantSession:
...
def test_the_mapper_returns_a_dto_not_a_coroutine():
    ...
    result = to_assistant_session_dto(_row())
...
def test_an_empty_optional_stays_none_rather_than_becoming_an_enum(field):
    ...
    dto = to_assistant_session_dto(_row(**{field: None}))
...
def test_the_dto_carries_no_case_data():
    ...
    fields = set(AssistantSessionDto.model_fields)

    assert not fields & {"context", "step", "unmatched_count", "pending_turn_message_id"}


def test_a_web_session_has_no_window_to_report():
    """Meta's 24-hour window is a WhatsApp fact; a web thread reports none rather than
    "closed", which the console would show as a warning."""
    dto = to_assistant_session_dto(_row(phone_e164=None))

    assert dto.window_open is None
    assert dto.phone_e164 is None
