...
from main.app.domain.channel.whatsapp.bot.surface import handoff_intent_for
from main.app.domain.communication.assistant.projection import ChannelState, channel_state
...
def test_every_handoff_action_has_a_portal_page_too(action):
    """The portal assistant answers the same handoffs with a page behind the customer's own
    login (D93); an action with no page would be announced and never linked."""
    from main.app.domain.communication.assistant.web import _HANDOFF_PAGES

    assert action in _HANDOFF_PAGES
