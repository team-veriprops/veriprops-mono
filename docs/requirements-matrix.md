...
| R4.4 | 4 | RBAC matrix (invite admins, approve agents, assign agents, approve payouts, configure pricing, resolve disputes, release reports) | R0.7 | user/auth/utils/permissions.py — `has_permission()`, `require_permission()`, `_ROLE_MATRIX` | no | yes | high | high | high | Every admin endpoint guarded; 49 permission-matrix unit tests (all role×permission combos) | unit | done |
| R4.5 | 4 | Admin team management (list, deactivate, change sub-role) | R4.4 | user/admin_team/ — `AdminTeamService`, `admin_team_router` (`GET /users/admin/team`, `POST /{id}/deactivate`, `PATCH /{id}/sub-role`) | no | yes | high | low | medium | Audit logs via ADMIN_DEACTIVATED + ADMIN_ROLE_CHANGED SecurityEvents; self-deactivation + last-SUPER guard enforced; 12 service unit tests | unit | done |
...
| R5.2 | 5 | Listing-URL parser (PropertyPro, Nigeria Property Centre; manual fallback) | R5.1 | verification/parser | no | yes | low | medium | medium | URL → fields populated; `POST /verifications/{id}/parse-listing`; 22 unit tests | unit | done |
...
| R5.9 | 5 | Payment methods — Card (Flutterwave /payments), Bank transfer (Paystack /charge virtual account), Wire (static SWIFT/IBAN + proof + admin confirm) | R5.5 | payment | yes | yes | high | high | high | All three branches wired + 17 payment service tests; webhook_router mounted; idempotency guard tested | unit | done |
...
| R5.15 | 5 | Customer upgraded to `trusted` on first successful payment | R5.9 | user/service | yes | yes | medium | low | low | `_on_payment_succeeded()` checks trust_status + calls `user_repo.update(trust_status=TRUSTED)` + TRUST_ELEVATED security event; tested via webhook idempotency test | unit | done |
