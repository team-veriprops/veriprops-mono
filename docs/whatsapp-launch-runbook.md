# WhatsApp channel — launch runbook (PRD §7.11)

Everything in §7.11 that a person has to do, in the order it has to be done. The code-side
...
- `WHATSAPP_PROVIDER=stub` everywhere except production. `ENVIRONMENT=prod` **requires**
  `meta` and the app refuses to boot otherwise, so there is no way to launch onto the stub by
  accident and no way for CI to reach Meta.
- The Doppler keys the live path needs, in the `prd` config of `veriprops-backend`:
  ...
  `WHATSAPP_BUSINESS_WEBHOOK_VERIFY_TOKEN`, and the §7.5 RS256 keypair.
...
checks this; it is the anti-impersonation anchor §7.1.2 rests on, and a customer's only
independent way to tell our number from a copy.

### 1.2 Template approval ⊘ (code-side: green)
All seven §7.7 templates are declared, bodied, and **sent by real code paths** — `otp_auth`,
...
asserts the customer gets the §7.6.5 apology plus a human promise, an admin gets
`BOT_PIPELINE_FAILED`, the fault is one-shot, and §7.10 counts it under `PIPELINE_FAILURE`.
...
The §7.8 clauses are live in three documents at version `1.1.0`: Platform Terms (WhatsApp as
...
- **Conversation-theme tracker** ⊘ — live from the first concierge chat. §7.10's escalation
...
| # | Do | Expect |
|---|---|---|
| 1 | Message the number from the website widget | §7.6.1 welcome: bot disclosure, payment pledge, five-item menu |
| 2 | Check `/admin/messages` → WhatsApp | The thread is there, `ChannelBadge` says WhatsApp, and §7.10 counted an enquiry with the widget's page code |
...
| 5 | Ask "is this property genuine?" | Refused and routed to a person — the §7.6.4 guardrail, on the live classifier |
...
| 8 | Drive a case to a milestone | The §7.6.2 template arrives — and only if utility consent is on |
| 9 | Reply STOP | Answered in one turn by the bot; both consents off in `/account/whatsapp` |
| 10 | Send a voice note | The §7.6.3 acknowledgement, flagged as audio in the console |
...
- Watch §7.10's seam conversion. It is the cost of the A1 trust boundary and the number the
