...
- **No leaking backend errors to the screen.** Render a failed request only through `getErrorMessage(err, fallback)` ([lib/errors.ts](src/lib/errors.ts)), never `err.message`:
  - **4xx** shows the backend's message, which is copy written for the user.
  - **5xx** shows `SERVER_ERROR_MESSAGE` plus `Ref: <reference>` (the ID the backend logged it under), never the server's text and never the caller's fallback, which may blame the user ("Email or password is incorrect").
  - **Offline / timeout** show their own words.
  - **Anything else** (a runtime error) shows the fallback.

  The backend already sanitises every 5xx; this is the second line of defence. `HttpError` carries `kind` (`response`/`network`/`timeout`), `httpStatus`, the backend's `code` and `reference`. `code` is the backend's error code, not the HTTP status. `isCredentialRejection(err)` (a 401) is the only failure the sign-in lockout counts ([login/lockout.ts](src/components/website/auth/login/lockout.ts)), so an outage never locks anyone out. `startOauthPopup`'s `onError` message is pre-sanitised copy, which is why its two callers render it directly.
...
Every sign-out control goes through `useSignOut()` ([libs/useSignOut.ts](src/components/website/auth/libs/useSignOut.ts)) — never `useLogoutMutation().mutate()` directly. It owns the busy overlay, the double-press guard and the redirect together, so the two controls (`TopNavUserMenu`, the `AppShell` drawer) cannot drift apart and a third would inherit all three for free. Three things there are deliberate:

- **The busy state is global, not on the button.** The desktop menu item unmounts on the very click that starts the sign-out — Radix closes the menu — so its `"Signing out…"` label is never seen. [SignOutOverlay](src/components/website/auth/SignOutOverlay.tsx) is mounted in `ClientWrapperProvider` beside `SessionRecoveryOverlay` and is what actually acknowledges the press.
- **It keys on `useAuthStore.signingOut` (user intent), never on the logout mutation's pending state.** `usePendingLogoutRetry` drives that same mutation in the background when a queued offline logout flushes; keying on `isPending` would flash a full-screen "Signing you out…" at a signed-in user. `signingOut` is in-memory only (kept out of `partialize`, so a tab killed mid-sign-out doesn't reopen stuck behind it), and `clear()` deliberately leaves it raised because it runs *before* the redirect.
- **The redirect is a full document load** (`navigateAfterSignOut`, [lib/session-navigation.ts](src/lib/session-navigation.ts)) — it holds the overlay up until the login page paints, and drops the TanStack cache, which otherwise survives a soft navigation and would serve the previous user's rows to the next sign-in in the same tab. A failsafe (`SIGN_OUT_MAX_WAIT_MS`) leaves anyway if the request never settles: the logout call sets no timeout and the overlay cannot be dismissed.
- **Leaving before the logout answered must not bounce back into the app.** The failsafe can leave while the HttpOnly session cookies are still in the browser, and `/auth/login` is guest-only in `proxy.ts`. So sign-out lands on `SIGNED_OUT_LOGIN_URL` ([lib/routes.ts](src/lib/routes.ts)): the login page with a marker the guard never bounces, and on no other guest-only page. The session is still ended, because `logoutLifecycle` queues the logout (`pendingLogout`, persisted) *before* the request goes out and dequeues it only on a real response, so the login page's `usePendingLogoutRetry` re-sends it. The backend revokes a device through the refresh cookie even after the access token has lapsed. `runSignOut` also clears the persisted session before leaving, since on the failsafe path the mutation never got to. `UAT-SESS-05` holds the logout call to prove all of this end to end.

Revoking *other* sessions (`/account/devices`) is not this — it keeps you signed in here, so those controls use the ordinary inline spinner instead, scoped per row.
...
A colour class naming a token the theme doesn't define (`bg-brand`, `text-brand-primary`) is **not** a build error in Tailwind v4 — it silently generates no CSS, which is how the 404 page shipped a white label on a transparent button. Use only the `--color-*` tokens declared in `theme.css`; [styles/theme-tokens.test.ts](src/styles/theme-tokens.test.ts) fails on any `*-brand-*` utility without one. Full-page dead ends (404, 403, 500) render through [StatusPage](src/components/ui/StatusPage.tsx), each passing its own `eyebrow` + lucide `icon` beside the copy so all three read in the landing page's editorial voice; the dot-grid/glow atmosphere behind it (and behind the hero and CTA) is [BrandBackdrop](src/components/ui/BrandBackdrop.tsx), not hand-rolled gradient divs; send a user "back to their dashboard" with `dashboardFor(user)` ([redirect.ts](src/components/website/auth/libs/auth/redirect.ts)), never a hardcoded surface. `proxy.ts` uses the same function with `{ fallback: ROUTES.HOME }` — keep that fallback: a signed-in session with no persona sent to `/portal` would be bounced by the portal guard straight back, an infinite redirect.
...
- **Toasts are Sonner, and only Sonner.** `import { toast } from "sonner"` → `toast.success(msg)` / `toast.error(msg, { description })`. The `<Toaster />` is mounted once in `ClientWrapperProvider` from [3rdparty/ui/sonner.tsx](src/components/3rdparty/ui/sonner.tsx). The app previously carried a second, shadcn/Radix toaster (`hooks/use-toast.ts` + `3rdparty/ui/toast*`) whose `toast({ title, variant })` object form is now **deleted** — for months its `<Toaster />` was the mounted one while 23 files called Sonner's, so 127 calls (54 of them errors) rendered nothing at all. Don't reintroduce a second system: if a toast doesn't appear, check the call, not the mount. Tests mock it as `vi.mock("sonner", () => ({ toast: { success: vi.fn(), error: vi.fn() } }))`.
...
- **Tags choose engines and lanes** ([e2e/playwright.config.ts](e2e/playwright.config.ts)).
  ...
  - A describe tagged **`@serial`** touches state other specs can see: the shared Mailpit inbox (`clearMailbox`), a seeded persona's data, or the seeded customer's drafts. `pnpm e2e` ([e2e/run-lanes.mjs](e2e/run-lanes.mjs)) runs two invocations: `UAT_LANE=parallel`, then `UAT_LANE=serial` (the `<engine>-serial` projects, one worker, reusing the first lane's seed and sessions via `UAT_REUSE_SEED`). The serial lane still runs when the parallel lane has failures, and either lane failing fails the run. Filter by engine with the plain name (`pnpm e2e --project=webkit-mobile`); the runner renames it to `webkit-mobile-serial` for the serial lane, which otherwise has no such project and fails the whole run. Project dependencies would skip it instead, so don't chain the lanes that way. Always run the suite through `pnpm e2e`: a bare `playwright test` would run both lanes at once. Anything not tagged `@serial` must own its data (a `scenario` fixture), so it stays parallel-safe.
...
- **Deterministic waits only.** `waitReady(page)` (`__app_ready__`) and web-first assertions — never `waitForTimeout`.
  After a navigation the page makes itself (a post-sign-in redirect, a form moving on), use
  `waitForPage(page, url, { timeout })` ([helpers/app.ts](e2e/helpers/app.ts)), never a bare `page.waitForURL`.
  `waitForURL` defaults to `load`, which also waits for every image and font, and that was SESS-04's flake.
  `waitForPage` waits for `domcontentloaded` and then `waitReady`, just as `goto` does for navigations a spec starts.
...
- **Let animations finish before an axe scan.** Axe reads computed colours, so a scan taken mid-transition (a
  ...
  Waiting can't cover a toast. It lives about 4 s and can start fading *during* the scan (seen as 2.19:1 against
  about 17:1 at rest), so page scans exclude `TOASTER_SELECTOR` ([helpers/a11y.ts](e2e/helpers/a11y.ts)). Toasts get
  one deliberate at-rest check instead, in `UAT-WA-04`, via `include: TOASTER_SELECTOR`. All toasts share one Sonner
  style, so that single check covers every one. Don't drop the exclusion to "catch more": it brings the flake back.
...
| App shell sign-out | `user-menu`, `user-menu-signout` (desktop top nav); `sidebar-open`, `sidebar-signout` (mobile drawer); `signout-overlay` (the busy state, either layout) — drive through `signOut(page)` in `e2e/helpers/ui.ts`, which picks the layout |
