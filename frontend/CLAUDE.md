...
- **Full-screen wizards** (submission, agent apply) share [src/components/ui/wizard/WizardOverlay.tsx](src/components/ui/wizard/WizardOverlay.tsx) — a stepper + footer shell; keep step content in per-step components and thread a `testIdPrefix`. Pass `closable={false}` to hide the close control for a compulsory flow the user cannot dismiss (e.g. the agent-onboarding gate below) — default is closable. The overlay is `z-45`: above persistent chrome (`z-40`), **below** every portaled Radix layer (`z-50`). Never raise it above `z-50` — a global mandatory modal (`ConsentReacceptanceModal`, `SessionRecoveryOverlay`) can open over a wizard, and Radix disables pointer events outside an open modal, so a wizard painted on top leaves the user a visible form they cannot use (pinned by `WizardOverlay.test.tsx`).
