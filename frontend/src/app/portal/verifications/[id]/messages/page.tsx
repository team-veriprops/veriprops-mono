...
export default function PortalVerificationMessagesPage({
  params,
}: {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["threads", "verification", id],
    queryFn: () => threadService.getByVerification(id),
    staleTime: 60_000,
  });
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-3 px-4 py-3 border-b border-gray-200 bg-white">
        ...
        <div>
          ...
          <p className="text-xs text-gray-500">Verification {id.slice(0, 8)}…</p>
        </div>
      </div>
      ...
      {thread && (
        <div className="flex-1 min-h-0">
          <ThreadView
            ...
            placeholder="Message the Veriprops team…"
          />
        </div>
      )}
    </div>
