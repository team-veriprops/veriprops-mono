...
import { useAssistantSessionQuery, useHandBackMutation } from "./libs/useAssistantQueries";

/**
 * Who is answering this thread, and the way back (PRD §26.6, §16.7, D57, D93).
 *
 * A thread goes sticky-`HUMAN` the moment an agent replies, and stays there until someone
 * hands it back. That is deliberate — the assistant talking over an agent mid-conversation
 * is the failure customers notice most — but it is also **invisible from the message
 * list**: a silent assistant looks exactly like a working one. This banner is what makes
 * the state legible and reversible, so an agent who answered one question doesn't
 * accidentally take a customer off the assistant forever.
 *
 * It carries a second invisible fact, but only on WhatsApp: Meta's 24-hour service window
 * (§26.7). Outside it, a reply typed here is queued behind a `window_reopen` template
 * rather than delivered as written — which an agent needs to know *before* writing a long
 * answer, not after. A web thread has no such window, so the chip never appears there.
 *
 * Admin-only: the endpoints behind it are `MANAGE_VERIFICATIONS`-gated, and a 403 from the
 * shared HTTP client hard-navigates to `/forbidden`. Render it only on an admin surface,
 * on any thread the assistant can answer — mount it wherever that surface can reply.
 */
export default function AssistantModeBanner({
  ...
  const { data: session, isLoading } = useAssistantSessionQuery(conversationId);
  const handBack = useHandBackMutation();

  if (!conversationId || isLoading || !session || !session.enabled) return null;
    ...
    >
      ...
      {isHuman && (
        <button
          ...
          onClick={() => handBack.mutate(conversationId)}
