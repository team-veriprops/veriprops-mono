...
export const lgaByState: Record<string, Array<{ value: string; label: string }>> = {
  'abia': [
  ...
  'fct': [
    ...
    { value: 'abuja-municipal', label: 'Abuja Municipal (AMAC)' },
  ...
  // Add remaining states with placeholder LGAs for brevity
  'adamawa': [
  ...
  'jigawa': [
    ...
    { value: 'kirika-samma', label: 'Kirika Samma' },
...
export function getLgasForState(stateValue: string): Array<{ value: string; label: string }> {
  return lgaByState[stateValue] || [];
}

export function getStateLabel(stateValue: string): string {
  const state = nigerianStates.find(s => s.value === stateValue);
  return state?.label || stateValue;
}

export function getLgaLabel(stateValue: string, lgaValue: string): string {
  const lgas = lgaByState[stateValue] || [];
  const lga = lgas.find(l => l.value === lgaValue);
  return lga?.label || lgaValue;
}
